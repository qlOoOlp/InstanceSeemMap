...
def build_visgraph_with_obs_map(obs_map, use_internal_contour=False, internal_point=None, vis=False):
    ...
    for c in contours_list:
        if len(c) >= 3:
            poly = Polygon(c)
            if poly.is_valid and poly.area > 10:  # 작은 self-intersecting 폴리곤 제거
...
def build_visgraph_with_obs_map2(obs_map, use_internal_contour=False, internal_point=None, vis=False):
    # obs_map = binary_opening(obs_map, structure=np.ones((1, 1)))
    # obs_map = binary_closing(obs_map, structure=np.ones((3, 3)))
    obs_map_vis = (obs_map[:, :, None] * 255).astype(np.uint8)
    obs_map_vis = np.tile(obs_map_vis, [1, 1, 3])
    if vis:
        cv2.imshow("obs", obs_map_vis)
    ...
    for c in contours_list:
        if len(c) >= 3:
            poly = Polygon([(p[1], p[0]) for p in c])  # Shapely expects (x, y)
            poly = poly.buffer(0)  # self-intersection 자동 보정
            if poly.area > 5:
                shapely_polygons.append(poly)
    ...
    for poly in merged:
        if poly.area < 15:
            continue
        coords = np.array(poly.exterior.coords).astype(int)
        contour_pos = [vg.Point(p[1], p[0]) for p in coords]  # row=y, col=x
        poly_list.append(contour_pos)

        # if vis:
        #     coords_cv2 = coords[:, [0, 1]]  # x, y
        #     cv2.drawContours(obs_map_vis, [coords_cv2.reshape(-1, 1, 2)], -1, (255, 0, 0), 2)


        # if vis:
        #     coords_cv2 = coords[:, [0, 1]].reshape(-1, 1, 2)
        #     obs_map_vis_copy = obs_map_vis.copy()
        #     cv2.drawContours(obs_map_vis_copy, [coords_cv2], -1, (255, 0, 0), 2)
        #     cv2.imshow("merged - each contour", obs_map_vis_copy)
        #     cv2.waitKey()
        if vis:
            coords_cv2 = coords[:, [0, 1]].reshape(-1, 1, 2)  # x, y for OpenCV
            cv2.drawContours(obs_map_vis, [coords_cv2], -1, (255, 0, 0), 2)  # 누적해서 그리기
            cv2.imshow("merged - accumulating", obs_map_vis)
            cv2.waitKey()
