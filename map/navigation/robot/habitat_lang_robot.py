...
from map.navigation.robot.lang_robot import LangRobot
from map.navigation.dataloader.habitat_dataloader import VLMapsDataloaderHabitat
from map.navigation.navigator.navigator import Navigator
from map.navigation.controller.discrete_nav_controller import DiscreteNavController
...
#     cvt_pose_vec2tf,
# )
from map.utils.index_utils import find_similar_category_id
from map.utils.habitat_utils import make_cfg, tf2agent_state, agent_state2tf, display_sample
...
class HabitatLanguageRobot(LangRobot):
    ...
    def __init__(self, config: DictConfig):
        ...
        self.test_scene_dir = self.config["habitat_dir"]
        # data_dir = Path(self.config["data_paths"]["vlmaps_data_dir"]) #/ "vlmaps_dataset"
        self.data_dir = os.path.join(self.config['data_dir'],self.config['data_type'],self.config['dataset_type'])
        ...
        self.camera_height = self.config["pose_info"]["camera_height"]
        ...
        self.nav = Navigator()
        self.controller = DiscreteNavController(self.config["controller_config"])
    ...
    def setup_scene(self, scene_id: int):
        ...
        vlmaps_data_dir = os.path.join(self.data_dir, scene_id)#self.vlmaps_data_save_dirs[scene_id]
        print(vlmaps_data_dir)
        self.scene_name = self.config['scene_id']#vlmaps_data_dir.name.split("_")[0]
        ...
        if self.config['potential_obstacle_names'] and self.config['obstacle_names']:
            print("come here")
            self.map.customize_obstacle_map(
                self.config['potential_obstacle_names'],
                self.config['obstacle_names'],
    ...
    def setup_map(self, vlmaps_data_dir: str):
        self.load_scene_map(vlmaps_data_dir, self.config)

        # # TODO: check if needed
        # if "3d" in self.config.map_type:
        #     self.map.init_categories(mp3dcat.copy())
        #     self.global_pc = grid_id2base_pos_3d_batch(self.map.grid_pos, self.cs, self.gs)

        self.vlmaps_dataloader = VLMapsDataloaderHabitat(vlmaps_data_dir, self.config, map=self.map)
    ...
    def _setup_sim(self, scene_name: str):
        ...
        if self.sim is not None:
            self.sim.close()
        self.test_scene = os.path.join(self.test_scene_dir, scene_name[:-2], scene_name[:-2] + ".glb")
        self.sim_setting = {
            "scene": self.test_scene,
            **self.config["sim_setting"],
        }
    ...
    def execute_actions(
        ...
        if vis:
            map = self.map.get_obstacle_cropped() #!get_customized_obstacle_cropped()
