...
from map.utils.navigation_utils import get_dist_to_bbox_2d
import pickle
...
class Map:
    def __init__(self, map_config: DictConfig, data_dir: str = ""):
        ...
        print(f"obstacles_cropped shape: {self.obstacles_cropped.shape}")
        # self.obstacles_new_cropped = None

    def _setup_paths(self, data_dir: Union[Path, str]) -> None:
        if data_dir: self.data_dir = data_dir
        else: self.data_dir = self.map_config['data_dir'] #os.path.join(self.map_config['data_dir'], self.map_config['data_type'],self.map_config['dataset_type'], self.map_config['scene_id'])
        self.dataset_dir = os.path.join(self.data_dir, self.map_config['data_type'], self.map_config['dataset_type'], self.map_config['scene_id'])
        self.map_dir = os.path.join(self.dataset_dir, "map", f"{self.map_config['scene_id']}_{self.map_config['version']}")
        self.rgb_dir = Path(self.dataset_dir) / "rgb"
        self.depth_dir = Path(self.dataset_dir) / "depth"
        self.semantic_dir = Path(self.dataset_dir) / "semantic"
        self.pose_path = Path(self.dataset_dir) / "poses.txt"
        try:
    ...
    def create(map_config: DictConfig) -> Map:
        from map.navigation.map import VLMap
