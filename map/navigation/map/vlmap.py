...
from map.utils.mapping_utils import load_3d_map
from map.navigation.map.map import Map
from map.utils.index_utils import find_similar_category_id, get_segment_islands_pos, get_dynamic_obstacles_map_3d
from map.utils.clip_utils import get_lseg_score
...
class VLMap(Map):
    ...
    def load_map(self, data_dir: str) -> bool:
        self._setup_paths(data_dir)
        print(self.data_dir)
        raise Exception("vlmap load_map must be deleted")
        # if self.map_config.pose_info.pose_type == "mobile_base":
    ...
    def get_pos(self, name: str) -> Tuple[List[List[int]], List[List[float]], List[np.ndarray], Any]:
        ...
        mask_2d = self.instance_dict[name]["mask"]
        print(f"target: {self.instance_dict[name]['category']}")
