...
def pixel_accuracy(eval_segm, gt_segm, ignore=[]): #^ 일반적인 total pacc 구하는 함수
    """
    ...
    cl, n_cl = extract_classes(gt_segm) # list of classes , number of classes
...
def mean_accuracy(eval_segm, gt_segm, ignore=[]): #^ 
    ...
    overlap, n_overlap = get_ignore_classes_num(cl, ignore) # ignore에 존재하는 class 중 실제 gt 내에 존재하는 class 들을 overlap으로 저장
    eval_mask, gt_mask = extract_both_masks(eval_segm, gt_segm, cl, n_cl)
