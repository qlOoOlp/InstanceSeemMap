...
from utils.parser import parse_args_roomcls
from omegaconf import OmegaConf
from map.postprocessing.roomclassification import RoomClsProcessor
...
def main():
    ...
    save_dir = os.path.join(config['root_path'], config['data_type'],config['dataset_type'],config['scene_id'],'map',
                 f"{config['scene_id']}_{config['version']}","04classificateRoom")
    check_dir(save_dir)
    RoomClsProcessor(config, save_dir).process()
